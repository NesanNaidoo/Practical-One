...
dist_tbl2<- flights |> 
  filter(month==1) |> 
  group_by(carrier) |> summarise(meanDist=mean(distance,na.rm=FALSE),sdDist=sd(distance,na.rm = FALSE)) |> arrange(meanDist)
...
```{r}
# The standard deviation formula requires at least two values.If a carrier has only one flight record, the standard deviation will be NA 

flights |> 
  filter(month == 1)  |> 
  group_by(carrier)  |> 
  summarise(count = n())  
#Carrier OO has only 1 flight hence why sd is NA

#if all of its flights had the same distance for a carrier, this means a standard deviation of 0 as there is no deviation in the distances
flights |> filter(month == 1)  |>  group_by(carrier)  |> 
  summarise(uniqDist = n_distinct(distance)) %>%
  filter(uniqDist == 1)

#We know Carrier OO has only 1 flight so the carriers having more than 1 flight, had the same distance for their respective flights
```
...
flights |> 
  ...
  summarise(avgdep_delay = mean(dep_delay, na.rm = TRUE)) |> 
  pivot_wider(
    id_cols = c("month"), # columns to keep as identifiers
    names_from = carrier, # column to spread (unique entries become columns)
    values_from = (avgdep_delay) # column to use for values (
  ) |> arrange(month)
```
...
delflight <-flights |> filter(dep_delay>0) |> filter(arr_delay<=0)
prop<-nrow(delflight)/nrow(flights)
prop*100
```
...
multi_air_route<-flights |> group_by(origin,dest) |> 
  summarise(uniqcarriers = n_distinct(carrier))|> 
  filter(uniqcarriers > 1)
multi_air_route

#For each such route, calculate the average arrival delay for each airline (exclude NAs). Find the names of these airlines.
avgArr_DelayAirline <- flights |> 
  semi_join(multi_air_routes, by = c("origin", "dest")) |>  
  group_by(origin, dest, carrier) |> 
  summarise(avg_arrdelay = mean(arr_delay, na.rm = TRUE)) |>
  left_join(airlines, by = "carrier") |>  
  arrange(origin, dest, avg_arrdelay)  |> relocate(name, .before = avg_arrdelay)
avgArr_DelayAirline


#For each such route, identify the airline with the worst and best average arrival delay.
best_worst_airlines <- avgArr_DelayAirline |> 
  group_by(origin, dest) |> 
  summarise(
    best_airline = name[which.min(avg_arrdelay)],  
    best_delay = min(avg_arrdelay, na.rm = TRUE),
    worst_airline = name[which.max(avg_arrdelay)], 
    worst_delay = max(avg_arrdelay, na.rm = TRUE)
  ) |> arrange(origin, dest)
best_worst_airlines
#Identify the route with the greatest difference between the best and worst performing airlines
great_delay<-best_worst_airlines |>group_by(origin, dest) |>  summarise(delay_diff = worst_delay - best_delay) |> arrange(desc(delay_diff))
great_delay
great_delay[1,]
...
jfk_atl
# This means EV flights are already significantly delayed before takeoff. The arrival delay follows naturally.
flights |> 
  filter(origin == "JFK", dest == "ATL",carrier=="EV")
#Only 1 Ev Flight

flights |> 
  filter(origin == "JFK", dest == "ATL", hour == 7)
#It is clear EV airline are the sole cause for the delays because other carriers had no where near the delay time for the same route leaving at the same time.
```
...
#all columns with missing entries, typos and any other inconsistencies in the dataset below
data |> select(gender,height,disease_status,glucose)
